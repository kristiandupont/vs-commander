...
- **`this.schedule(cb)`** — runs after DOM nodes are created but _before_ they are inserted into the document. Useful for setting up properties without visual flicker.
...
The `copy` prop with a string value can exclude specific props from being re-applied on subsequent renders. `copy="!value"` tells Crank to re-apply all props _except_ `value`, letting the browser manage the input’s value:
...
function Modal({ children }) {
  return (
    <Portal root={document.getElementById("modal-root")}>{children}</Portal>
  );
}
```
